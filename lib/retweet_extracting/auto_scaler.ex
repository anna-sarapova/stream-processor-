defmodule RetweetExtracting.AutoScaler do
  use GenServer
  require Logger

  def start_module() do
    IO.inspect("Starting Retweet Auto Scaler")
    count_tweets = 0
  ...
  def handle_info(:work, count_tweets) do
    ...
    if rem(count_tweets, 4) > 0 do
      ...
      active_children = DynamicSupervisor.count_children(RetweetExtracting.PoolSupervisor).active
      desired_worker_nr = worker_number - active_children
      RetweetExtracting.PoolSupervisor.start_worker(desired_worker_nr)
    else
